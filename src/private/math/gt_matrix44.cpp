...
namespace gt
{
	...
	namespace math
	{
		...
		static constexpr f32 matrix44_identity[] =
		{
			1.f, 0.f, 0.f, 0.0f,
			0.f, 1.f, 0.f, 0.f,
			0.f, 0.f, 1.f, 0.f,
		...
		static constexpr f32 matrix44_empty[] =
		{
			0.f, 0.f, 0.f, 0.f,
			0.f, 0.f, 0.f, 0.f,
			0.f, 0.f, 0.f, 0.f,
		...
		/// 単位行列.
		/// </summary>
		const Matrix44& Matrix44::IDENTITY = *reinterpret_cast<const Matrix44*>( matrix44_identity );

		/// <summary>
		/// 空行列.
		/// </summary>
		const Matrix44& Matrix44::EMPTY = *reinterpret_cast<const Matrix44*>( matrix44_empty );
		...
		Matrix44::Matrix44( const Matrix33& M ) 
			: Matrix44( 
			M._11, M._12, M._13, 0.f,
			M._21, M._22, M._23, 0.f,
			M._31, M._32, M._33, 0.f,
			0.f, 0.f, 0.f, 1.f )
		{ }
...
#pragma endregion
		...
		Matrix44::self_type Matrix44::getInverse() const
		{
			...
			const value_type det = a0*b5 - a1*b4 + a2*b3 + a3*b2 - a4*b1 + a5*b0;
...
#pragma region SetterRotation
		...
		Matrix44::self_type& Matrix44::setRotation(Radian angle, const vector3_type& axis )
		{
			return this->setRotation( quaternion_type{ axis, angle } );
		}
...
#pragma endregion
		...
		Matrix44::self_type Matrix44::Slerp( const self_type& start, const self_type& end, value_type amount )
		{
#if 1
...
#else
			// 拡大縮小の補間.
			Vector3 s = Vector3::Lerp( start.getScale(), end.getScale(), amount );

			// 回転の補間.
			Quaternion r = Quaternion::Slerp( Quaternion::Rotate( start ).normalize(), Quaternion::Rotate( end ).normalize(), amount );

			// 位置の補間.
			Vector3 p = Vector3::Lerp( start.getPosition(), end.getPosition(), amount );

			return Matrix44().setScale( s ) * r.rotate() * Matrix44().setTranslate( p );
#endif
		}
