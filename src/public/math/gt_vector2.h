...
namespace gt::math
{
	...
	public:
		...
		/// <param name="target">ターゲット座標.</param>
		/// <returns>距離を返す.</returns>
		static value_type DistanceSqr(const Vector2& my, const Vector2& target) noexcept
		{
		...
		static Vector2 Velocity(const Vector2& my, const Vector2& target, const value_type speed = 1.0f) noexcept
		{
		...
		/// <returns>移動量を返す</returns>
		static Vector2 Velocity(Radian radian, const value_type speed = 1.0f) noexcept
		{
		...
		static Vector2 Lerp(const Vector2& start, const Vector2& end, const value_type t) noexcept
		{
		...
		static Vector2 LinearEquation(const Vector2& P, const Vector2& V, const value_type t) noexcept
		{
		...
		/// <returns>角度を返す.</returns>
		static CONSTEXPR_CPP20 value_type InnerDegree(const Vector2& v1, const Vector2& v2)
		{
		...
		/// <param name="pM">座標変換行列.</param>
		/// <returns>変換後の法線ベクトル.</returns>
		static Vector2 TransformCoord(const Vector2& target, const Matrix44& M) noexcept;
	...
	public:
		...
		constexpr Vector2 getNormalized() const noexcept
		{
		...
		/// <returns>距離を返す.</returns>
		CONSTEXPR_CPP20 value_type distance(const Vector2& endPoint) const
		{
		...
		/// <param name="endPoint">終点.</param>
		/// <returns>距離を返す.</returns>
		constexpr value_type distanceSqr(const Vector2& endPoint) const noexcept
		{
			return (*this - endPoint).lengthSqr();
		}
		...
		constexpr Vector2& lerp(const Vector2& start, const Vector2& end, value_type amount) noexcept
		{
		...
		/// <param name="amount">補間値(0.0~1.0).</param>
		/// <returns>補間後の参照を返す.</returns>
		constexpr Vector2& quadratic(const Vector2& V1, const Vector2& V2, const Vector2& V3, const value_type amount)
		{
		...
		constexpr Vector2& refract(const Vector2& I, const Vector2& N, const value_type Eta)
		{
			Vector2 T1 = -I;
			const value_type cosI = T1.dot(N);
			const value_type cosT2 = 1.f - Eta * Eta * (1.0f - cosI * cosI);
			...
			T1 = I * Eta;
			const Vector2 T2 = N * (Eta * cosI - std::sqrt(std::abs(cosT2)));
			*this = T1 + T2;
			return	*this;
		}
		...
		/// <param name="V">ベクトル.</param>
		/// <returns>角度を返す.</returns>
		CONSTEXPR_CPP20 value_type degree(const Vector2& V) const
		{
		...
		/// 方位角を求める.
		/// </summary>
		CONSTEXPR_CPP20 value_type getDirection() const noexcept
		{
		...
		CONSTEXPR_CPP20 Vector2& fromDirection(Radian direction) noexcept
		{
		...
		/// <returns>乗算後のベクトルを返す.</returns>
		constexpr Vector2& operator *= (const value_type other) noexcept
		{
			this->x *= other;
			this->y *= other;
			return	*this;
		...
		/// <param name="right">右辺値.</param>
		/// <returns>乗算後のベクトルを返す.</returns>
		constexpr Vector2&& operator * (const value_type right) && noexcept
		{
			return std::move(*this *= right);
		}
		...
		/// <param name="right">右辺値.</param>
		/// <returns>乗算後のベクトルを返す.</returns>
		constexpr Vector2 operator * (const value_type right) const& noexcept
		{
			return
			{
				this->x * right,
		...
		/// <param name="right">右辺値.</param>
		/// <returns>乗算後のベクトルを返す.</returns>
		friend Vector2 operator * (const value_type left, const Vector2& right) noexcept
		{
			return	right * left;
		}
