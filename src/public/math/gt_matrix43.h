/// @file		gtlib/math/gt_matrix43.hpp
/// @date		2014/12/09
/// @brief		4行3列行列構造体宣言
...
namespace gt
{
	...
	namespace math
	{
		...
		namespace tl
		{
			...
			/// <para>平行移動・回転.</para>
			/// </summary>
			template < typename ValueType, typename SizeType > struct Matrix43
			{
...
#pragma warning(pop)
				...
				/// 要素の先頭を示すポインタを取得.
				/// </summary>
				CONSTEXPR_CPP14 value_type* begin( void )
				{
				...
				/// 要素の先頭を示すポインタを取得.
				/// </summary>
				CONSTEXPR_CPP11 value_type const* begin( void ) const
				{
					return std::begin( this->v );
				}
				...
				/// 要素の最後の次を示すポインタを取得.
				/// </summary>
				CONSTEXPR_CPP14 value_type* end( void )
				{
				...
				/// 要素の最後の次を示すポインタを取得.
				/// </summary>
				CONSTEXPR_CPP11 value_type const* end( void ) const
				{
					return std::end( this->v );
				}
